...
def retiles(tasks_albers, tasks_wgs84, tile, pathin, pathout):
    ...
    for feat in lyr_wgs84_tasks:
        ...
        tif_data = pathin + '/' + tif[0:4] + '/' + tif[-4:] + '/' + tif + '/' + tif + '_loss_year_n0_m1.tif'
    ...
    if len(tifs) == 1:
        ...
        #expression = "{} = '{}'".format('tag', tile)
        #ds_clip = gdal.Warp('', ds_proj, format='MEM', cutlineDSName=tasks_albers, cutlineWhere=expression, cropToCutline=True, srcNodata=nodata, dstNodata=nodata)
        ...
        out_data = pathout + '/' + tile + '_loss.tif'
...
def main():
    ...
    for i in mpi_data_divide:
        ...
        try:
            retiles(args.tasks_albers, args.tasks_wgs84, i, args.input, args.output)
        except:
