...
def stat(tile, pathin_tcc, pathin_para, mask, contains_tiles, intersects_tiles, pathout):
    ...
    if tile in contains_tiles:
        ...
        df_out = pd.DataFrame(columns=['lon', 'mean', 'counts'])
        ...
        for i in range(len(boundary_step)-1):
            ...
            tcc_mask_array_2 = np.ma.masked_where(tcc_mask_array_1 >= 30, tcc_mask_array_1)

            counts_value = tcc_mask_array_2.count()
            ...
            value_dict['counts'] = [counts_value]
