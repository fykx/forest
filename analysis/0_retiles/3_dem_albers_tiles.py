...
def retiles(tasks_albers, tasks_wgs84, tile, pathout):
    ...
    for feat in lyr_wgs84_tasks:
        tif_data = feat.GetField('location')

        if os.path.exists(tif_data):
            tifs.append(tif_data)
        else:
            print(tif_data, ' not exist')
    ...
    if len(tifs) == 1:
        ...
        ds_proj = gdal.Warp('', tifs[0], format='MEM', xRes=30, yRes=30, srcSRS=proj_tif, dstSRS=spatialRef_albers_tasks_wkt, srcNodata=nodata, dstNodata=-9999)

        ds_clip = gdal.Warp('', ds_proj, format='MEM', outputBounds=(point_1[0],point_3[1],point_3[0],point_1[1]), outputBoundsSRS=spatialRef_albers_tasks_wkt, xRes=30, yRes=30, srcSRS=spatialRef_albers_tasks_wkt, dstSRS=spatialRef_albers_tasks_wkt, srcNodata=nodata, dstNodata=-9999)
        
        out_data = pathout + '/' + tile + '_dem.tif'
        ...
        ds_proj = gdal.Warp('', ds_mosaic, format='MEM', xRes=30, yRes=30, srcSRS=proj_tif, dstSRS=spatialRef_albers_tasks_wkt, srcNodata=nodata, dstNodata=-9999)
...
def main():
    ...
    if comm_rank == 0:
        datas = read_tiles(args.tasks_albers)
    ...
    for i in mpi_data_divide:
        ...
        try:
            retiles(args.tasks_albers, args.tasks_wgs84, i, args.output)
        except:
            print(i, 'Processing failed')
